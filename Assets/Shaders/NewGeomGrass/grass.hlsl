...
void geom(uint primitiveID : SV_PrimitiveID, triangle Varyings input[3], inout TriangleStream<GeometryOutput> triStream)
{
    ...
    for (uint blade = 0; blade < grassBlades; blade++)
    {
        ...
        float3 positionWS = (1 - sqrt(r1)) * input[0].positionWS + (sqrt(r1) * (1 - r2)) * input[1].positionWS + (
            sqrt(r1) * r2) * input[2].positionWS;
        ...
        float2 wind = float2(sin(_Time.y + positionWS.x * 0.5), cos(_Time.y + positionWS.z * 0.5)) * _WindStrength *
            sin(_Time.y + r);
